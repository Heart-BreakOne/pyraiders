...
def change_priority():
    ...
    user_account = input("Enter the account name or the account Id you want to change the unit priority for: ")
    accounts = open_file(constants.py_accounts)

    for account in accounts:
        if account["name"] == user_account or account["userId"] == user_account or account["otherUserId"] == user_account:
            for unit in account["units"]:
                print(f'UNIT ID: {unit["unitId"]}   Current priority: {unit["priority"]}.    Unit: {unit["unitType"]}. Level (not consistent yet): {unit["level"]}. Specialization: {unit["specializationUid"]}')
            break
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 helper_tools.py <command>\nCommands:\nadd_account or a\nchange_priority or c\nload_browser or l")
        sys.exit(1)
